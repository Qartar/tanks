...
void cGame::m_StartServer ()
{
	...
	if ( !m_bDedicated )
	{
		m_clients[0].active = true;
		m_clients[0].local = true;
		...
		strncpy( m_clients[0].name, cls.name, SHORT_STRING );

		m_World.AddObject( &m_Players[0] );
	}
	else
	{
		m_clients[0].active = false;
		m_clients[0].local = false;
	}
...
void cGame::m_StopServer ()
{
	...
	for ( i=0,cl=m_clients ; i<MAX_PLAYERS ; i++,cl++ )
	{
		if ( cl->local )
			continue;
		if ( !cl->active )
	...
	while ( true )
	{
		...
		if ( socket == NS_SERVER )
		{
			...
			case clc_command:
				...
				break;
			case clc_disconnect:
				m_WriteMessage( va("%s disconnected.", m_clients[m_netclient].name ) );
				m_ClientDisconnect( m_netclient );
				break;
			case clc_say:
				string = m_netmsg.ReadString( );
				m_WriteMessage( va( "%s: %s", m_clients[m_netclient].name, string ) );
			default:
				return;
			}
		}
...
//		return;
	...
	while ( true )
	{
		...
		m_Players[i].vColor.r	= m_netmsg.ReadByte( ) / 255.0f;
		m_Players[i].vColor.g	= m_netmsg.ReadByte( ) / 255.0f;
		m_Players[i].vColor.b	= m_netmsg.ReadByte( ) / 255.0f;
...
void cGame::m_WriteFrame ()
{
	...
	for ( i=0,cl=m_clients ; i<MAX_PLAYERS ; i++,cl++ )
	{
		...
		message.WriteByte( m_Players[i].vColor.r * 255 );
		message.WriteByte( m_Players[i].vColor.g * 255 );
		message.WriteByte( m_Players[i].vColor.b * 255 );
...
void cGame::m_ConnectToServer ()
{
	...
	pNet->Print( NS_CLIENT, m_netserver, va( "connect %s %i", cls.name, m_netchan.netport ) );
}
...
void cGame::m_ClientConnect ()
{
	...
	sscanf( m_netstring, "%s %s %i", tempbuf, clname, &netport );
	...
	for ( i=0 ; i<MAX_PLAYERS ; i++ )
	{
		...
		if ( m_clients[i].netchan.address == m_netfrom )
		{
			// this client is already connected
//			if ( m_clients[i].netchan.netport == netport )
				return;
		}
	}
	...
	pNet->Print( NS_SERVER, cl->netchan.address, "connect" );
	...
	m_Broadcast_Print( va( "%s connected.", cl->name ) );

	m_WriteInfo( i, &message );
...
void cGame::m_WriteInfo (int client, netmsg_t *message)
{
	...
	message->WriteByte( m_clients[client].active );
	message->WriteString( m_clients[client].name );
...
void cGame::m_ReadInfo ()
{
	...
	m_clients[client].active = ( active == 1 );
	
	string = m_netmsg.ReadString( );
	strncpy( m_clients[client].name, string, SHORT_STRING );
}
...
void cGame::m_ClientDisconnect (int nClient)
{
	...
	if ( !m_clients[nClient].active )
		return;

	m_World.DelObject( &m_Players[nClient] );
	m_clients[nClient].active = false;
...
void cGame::m_InfoSend ()
{
	...
	pNet->Print( NS_SERVER, m_netfrom, "info" );
}
